...
export default function AdminCoursesPage() {
  ...
  return (
    <EnginePageLayout
      ...
      recentActivity={courses.slice(0, 4).map((c) => ({
        label: c.title || "Untitled",
        meta: `${c.subject} • ${c.classLevel}`,
        timestamp: c.status ? `Status: ${c.status}` : undefined
      }))}
    >
      <div>
        <form onSubmit={onSubmit} className="grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-4 sm:grid-cols-2">
